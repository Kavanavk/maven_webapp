pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sshagent(['tomCat']) {
                    sh 'scp -o StrictHostKeyChecking=no target/demo.war ubuntu@40.192.1.202:/home/ubuntu/'
                    sh 'ssh ubuntu@40.192.1.202 "sudo mv /home/ubuntu/demo.war /opt/tomcat/webapps/"'
                }
            }
        }
    }
